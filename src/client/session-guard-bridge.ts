...
export async function sessionGuardStopNextTurn(sessionId: string): Promise<boolean> {
  return callGuard(sessionId, 'stopNextTurn')
}

/** 尽力调用 sessionGuard.resume。失败静默。 */
export async function sessionGuardResume(sessionId: string): Promise<boolean> {
  return callGuard(sessionId, 'resume')
}

async function callGuard(sessionId: string, action: string): Promise<boolean> {
  try {
