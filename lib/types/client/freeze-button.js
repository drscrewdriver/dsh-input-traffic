...
import { sessionGuardResume, sessionGuardStopNextTurn } from "./session-guard-bridge.js";
 ...
 */
export function FreezeButton({ useSession, updateQueue, cancel, send, sendSteer, sessionId, setComposerBlock, notify, t }) {
    ...
    const freeze = async () => {
        ...
        const pending = rows.flatMap(row => row.text === null ? [] : [{ text: row.text, tier: row.placement === 'steering' ? 'safe_point' : 'queue' }]);
        // Detach every pending row: the running turn finishes naturally, then the
        // driver finds no pending work and stops.
        await Promise.all(rows.map(row => updateQueue(row.id, { kind: 'remove' }).catch(() => undefined)));
        ...
        if (sessionId !== undefined)
            void sessionGuardStopNextTurn(sessionId);
    };
    const resume = async () => {
        ...
        try {
            ...
            if (sessionId !== undefined)
                await sessionGuardResume(sessionId);
            ...
            for (const entry of pending) {
                if (entry.tier === 'force')
                    await cancel();
                if (entry.tier === 'safe_point' && sendSteer !== undefined)
                    await sendSteer(entry.text);
                else
    ...
    return (_jsx("button", { type: "button", className: css.freeze, "aria-label": frozen ? t('steer.resume') : t('steer.freeze'), "aria-pressed": frozen || undefined, title: frozen ? t('steer.frozen') : undefined, onClick: () => {
            if (frozen)
                void resume();
            else
                void freeze();
        }, children: _jsx("span", { className: css.label, children: frozen ? t('steer.resume') : t('steer.freeze') }) }));
}
//# sourceMappingURL=freeze-button.js.map